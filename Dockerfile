# --- VERSIONS UPDATED TO 2025 STANDARDS ---
ARG ALPINE_VER="3.22"
...
# --- FINAL STAGE (ARM ONLY) ---
# NOTE: Using alpine 3.22 to match build args
...
# 1. Download S6 Overlay for ARM64 (AARCH64)
# We use AARCH64 because you are on an Oracle Ampere VM
...
# 2. Install dependencies (REMOVED standard ffmpeg)
# Added 'wget' just in case, though we used ADD above.
RUN apk --no-cache add redis ca-certificates openssl pcre zlib envsubst uuidgen postgresql \
    postgresql-client postgresql-contrib wget

# 3. Copy Static FFmpeg (Crucial for playback on ARM)
